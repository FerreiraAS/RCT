TABLE.2b <- function(dataset, variables, covariate, bw.factor, control.g, wt.labels, missing = c("complete.cases", "mean.imputation", "multiple.imputation"), 
	...
	interaction <- paste("F(", format(round(mod1.aov[4, 1], digits = 0), nsmall = 0), ",", format(round(mod1.aov[4, 2], digits = 0), nsmall = 0), 
		")=", format(round(mod1.aov[4, 3], digits = 3), nsmall = 3), ", p", p.value, flag, sep = "")
	...
	for (i in 2:3) {
		...
		model.res[i - 1, ] <- paste("F(", format(round(mod1.aov[i, 1], digits = 0), nsmall = 0), ",", format(round(mod1.aov[i, 2], digits = 0), nsmall = 0), 
			")=", format(round(mod1.aov[i, 3], digits = 3), nsmall = 3), ", p", p.value, flag, sep = "")
	}
	...
	bw.pvalues <- c()
	smd.values <- c()
	mult.between <- summary(pairs(emmeans(mod1, ~GROUP_M | TIME_M), reverse = FALSE), infer = c(TRUE, TRUE))
	for (i in 1:(length(wt.labels))) {
		...
		ifelse(p.values > 0.001 & p.values < alpha, bw.pvalues <- c(bw.pvalues, paste(format(round(p.values, digits = 3), nsmall = 3), "*", sep = "")), 
			"")
	...
	print(paste("Table 2b: Two-way linear mixed model analysis (", toString(variables), ").", sep = ""), quote = FALSE)
