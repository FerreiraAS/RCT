TABLE.3 <- function(dataset, variables, bw.factor, control.g, wt.labels, alpha, n.digits) {
	# LAST MODIFIED: DEZEMBER 13, 2021
	...
	for (i in 1:length(wt.labels)) {
		...
		# ridit analysis
		r1 <- meanridits(table(dataset[, i], bw.factor), margin = 2, ref = control.g)[levels(bw.factor) == control.g]
		r2 <- meanridits(table(dataset[, i], bw.factor), margin = 2, ref = control.g)[levels(bw.factor) != control.g]
		se.diff <- seriditdiff(table(dataset[, i], bw.factor)[levels(bw.factor) == control.g], table(dataset[, i], bw.factor)[levels(bw.factor) != 
			control.g])
		...
		z.p <- rbind(z.p, paste("z=", format(round(z.score, digits = 3), nsmall = 3), ", p", p.value, sep = ""))
