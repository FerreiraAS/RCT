FIGURE.1 <- function(dataset, variables, covariate, bw.factor, wt.labels, missing, m.imputations, xlabs, ylab, alpha) {
	...
	interaction <- parse(text = paste0('Interaction: ~ F[', format(round(mod1.aov[4, 1], digits = 0), nsmall = 0), '*","*', format(round(mod1.aov[4, 2], digits = 0), nsmall = 0), '] == ', format(round(mod1.aov[4, 3], digits = 3), nsmall = 3), '*","*', ' ~ p', p.value))
	...
	if (mod1.aov[4, 4] < alpha) {
		mtext(interaction, line = -16, outer = FALSE, cex = 0.75)
	}
	# plot legend
