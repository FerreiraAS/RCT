TABLE.2a <- function(dataset, variables, covariate, bw.factor, control.g, wt.labels, missing = c("complete.cases", "mean.imputation", "multiple.imputation"), 
	...
	mix.mod.res <- matrix("", nrow = length(head(t.labels, -1)), ncol = nlevels(bw.factor) * length(wt.labels))
	...
	wt.diff <- matrix("", nrow = length(t.labels), ncol = nlevels(bw.factor) * (length(wt.labels) - 1))
	...
	if (missing == "multiple.imputation") {
		...
		OUTCOME_M[which(is.na(OUTCOME_M) & TIME_M == levels(TIME_M)[1])] <- mean(OUTCOME_M[which(!is.na(OUTCOME_M) & TIME_M == levels(TIME_M)[1])])
	...
	if (missing != "multiple.imputation") {
		if (!is.null(covariate)) {
			mod1 <- lme(fixed = OUTCOME_M ~ TIME_M * GROUP_M + COVARIATE_M, random = ~1 | ID_M/TIME_M, data = data_M)
		} else {
		...
		if (!is.null(covariate)) {
			mod1 <- with(data = imp, lme(fixed = OUTCOME_M ~ TIME_M * GROUP_M + COVARIATE_M, random = ~1 | ID_M/TIME_M))
	...
	for (i in 1:length(wt.labels)) {
		for (j in 1:nlevels(bw.factor)) {
			# dados válidos
			N <- rbind(N, (paste("(n = ", sum(!is.na(OUTCOME_ORIG[TIME_M == i & GROUP_M == levels(bw.factor)[j]])), ")", sep = "")))
			# média (DP)
			desfecho <- rbind(desfecho, paste(format(round(mean(OUTCOME_M[TIME_M == i & GROUP_M == levels(bw.factor)[j]], na.rm = TRUE), digits = n.digits), 
				nsmall = n.digits), " (", format(round(sd(OUTCOME_M[TIME_M == i & GROUP_M == levels(bw.factor)[j]], na.rm = TRUE), digits = n.digits), 
				nsmall = n.digits), ")", sep = ""))
		}
	}
	...
	interaction <- paste("Interaction: F(", format(round(mod1.aov[4, 1], digits = 0), nsmall = 0), ",", format(round(mod1.aov[4, 2], digits = 0), 
		nsmall = 0), ") = ", format(round(mod1.aov[4, 3], digits = 3), nsmall = 3), ", p", p.value, flag, sep = "")
	...
	for (i in 2:3) {
		...
		model.res[i - 1, ] <- paste("Main effect: F(", format(round(mod1.aov[i, 1], digits = 0), nsmall = 0), ",", format(round(mod1.aov[i, 2], digits = 0), 
			nsmall = 0), ") = ", format(round(mod1.aov[i, 3], digits = 3), nsmall = 3), ", p", p.value, flag, sep = "")
	}
	model.res[3, 1] <- interaction

	# calcula e preenche a subtabela WITHIN-GROUP (SAME LINEAR MIXED MODEL)
	mult.within <- summary(pairs(emmeans(mod1, ~TIME_M | GROUP_M), reverse = FALSE), infer = c(TRUE, TRUE))
	wt <- c()
	wt.pvalues <- c()
	for (i in 1:nlevels(bw.factor)) {
		...
		for (k in 1:(length(wt.labels) - 1)) {
			...
			ifelse(p.values[k] > 0.001 & p.values[k] < alpha, wt.pvalues <- c(wt.pvalues, paste(format(round(p.values[k], digits = 3), nsmall = 3), 
				"*", sep = "")), "")
			ifelse(p.values[k] > alpha, wt.pvalues <- c(wt.pvalues, format(round(p.values[k], digits = 3), nsmall = 3)), "")
		}
	}
	...
	for (i in 2:(length(wt.labels))) {
		...
		if (missing != "multiple.imputation") {
			...
			estimate <- round(confint(mod2.sum, level = 1 - alpha)$confint[, "Estimate"], digits = n.digits)
			low.ci <- round(confint(mod2.sum, level = 1 - alpha)$confint[, "lwr"], digits = n.digits)
			...
			pred["CHANGE_M", "ID"] <- -2
			imp <- mice(data = df, pred = pred, method = "2l.pan", m = m.imputations, seed = 0, print = FALSE)
			implist <- mids2mitml.list(imp)
			mod2 <- with(data = implist, lm(CHANGE_M ~ bw.factor + BASELINE_M, random = ~1 | ID))
			...
			low.ci <- round(confint.mitml.testEstimates(mod2.sum, level = 1 - alpha)[1, 1], digits = n.digits)
			upp.ci <- round(confint.mitml.testEstimates(mod2.sum, level = 1 - alpha)[1, 2], digits = n.digits)
	...
	bw.diff[1, ] <- rep(paste(wt.labels[-1], wt.labels[1], sep = " - "), by = length(wt.labels) - 1)
